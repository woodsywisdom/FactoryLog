...
def read_docs(file_path):
    ...
    # Add a new class within 'items' used to keep track of the total power produced
    data_dict['items'].update({'Power_Produced': {'name': 'Power', 'points': 0.0}}) 
    
    #Create a list of just the items useable as ingredients
...
def load_recipes(classes, recipes, all_items, machines):
    for data in classes:
        ...
        #Regex filtering for recipes that are not built with the construction tool
...
# Helper function taking the 'mProduct' key of a recipe class and returning a list of recipe products as 2 item dicts {'item', 'amount'}
...
# Same as load_machines but finds the average power consumption of machines with fluctuating power and returning that in the same 'power_use' key
